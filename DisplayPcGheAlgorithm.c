...
/***************************************************************
 * @brief To calculate AggressivenessFactor based on GHE operation mode and Ambient lux
 *
 * @param pGheCfg
 * @param AmbientLux
 * @return double
 ***************************************************************/
double DisplayGheCalculateAggressivenessFactor(GHE_CONFIG *pGheCfg, uint32_t AmbientLux)
{
    double AggressivenessFactor = 0, AmbientLuxFactor = 0;
    uint32_t Lux1 = 0, Lux2 = 0;
    uint8_t AggrPercent1 = 0, AggrPercent2 = 0;
    const uint32_t LastIndex = pGheCfg->GheTriggerConfig.NumEntries - 1;

    if (GHE_TRIGGER_FIXED_AGGRESSIVENESS == pGheCfg->GheTriggerConfig.Trigger) // Fixed AggressivePercent Mode, no Als Lux value required.
    {
        AggressivenessFactor = ((double)pGheCfg->GheTriggerConfig.FixedAggressivenessPercent / 100);
        goto Exit;
    }

    if (AmbientLux < pGheCfg->GheTriggerConfig.LuxToAggrMappingTable[0].Lux)
    {
        AggressivenessFactor = 0;
        goto Exit;
    }

    if (AmbientLux >= pGheCfg->GheTriggerConfig.LuxToAggrMappingTable[LastIndex].Lux)
    {
        AggressivenessFactor = (double)pGheCfg->GheTriggerConfig.LuxToAggrMappingTable[LastIndex].AggressivenessPercent / 100;
        goto Exit;
    }

    // Search of two adjacent points for AmbientLux and get AggressivenessFactor
    for (uint32_t Index = 1; Index < pGheCfg->GheTriggerConfig.NumEntries; Index++)
    {
        if (pGheCfg->GheTriggerConfig.LuxToAggrMappingTable[Index].Lux >= AmbientLux)
        {
            Lux2         = pGheCfg->GheTriggerConfig.LuxToAggrMappingTable[Index].Lux;
            AggrPercent2 = pGheCfg->GheTriggerConfig.LuxToAggrMappingTable[Index].AggressivenessPercent;

            Lux1         = pGheCfg->GheTriggerConfig.LuxToAggrMappingTable[Index - 1].Lux;
            AggrPercent1 = pGheCfg->GheTriggerConfig.LuxToAggrMappingTable[Index - 1].AggressivenessPercent;

            AmbientLuxFactor = ((double)AmbientLux - (double)Lux1) / (double)(Lux2 - Lux1);

            // Interpolation for correction at that lux value and handling conversion of percentage
            AggressivenessFactor = ((AmbientLuxFactor * ((double)AggrPercent2 - (double)AggrPercent1)) + (double)AggrPercent1) / 100;
            break;
        }
    }
...
double DetectFullScreenSolidColor(double *pPowerHistogram, double TotalPower, uint32_t *pHistogram, uint32_t ImageSize, bool InHdrMode)
{
...
static void DisplayGheAlgorithmCore_v1_0(DISPLAY_PC_XPST_CONTEXT *pDpstContext)
{
    DISPLAY_PC_CONTEXT *pDisplayPcContext;
    ...
    pDisplayPcContext = pDpstContext->pDisplayPcContext;
    ...
    DetectFullScreenSolidColor(pGheParams->PowerDistribution, SumPower, pDpstContext->Algorithm.XpstAlgorithmDynamicData.Histogram, pDpstContext->Algorithm.XpstAlgorithmStaticData.ImageSize, FALSE);
...
static double GetRelativeFrameBrightnessChange(DISPLAY_PC_XPST_CONTEXT *pDpstContext)
{
	uint32_t *prevhist = pDpstContext->Algorithm.XpstAlgorithmDynamicData.PrevHistogram;
	uint32_t *currhist = pDpstContext->Algorithm.XpstAlgorithmDynamicData.Histogram;

        double frameaveragepowerprev = 0;
        double frameaveragepowercurr = 0;
        double histchangepercentage = 0;

        for (uint32_t count = 0; count < XPST_BIN_COUNT; count++) {
                frameaveragepowerprev += pDpstContext->Algorithm.XpstAlgorithmStaticData.DeGammaLUT[count] * (double)prevhist[count];
                frameaveragepowercurr += pDpstContext->Algorithm.XpstAlgorithmStaticData.DeGammaLUT[count] *
                        (double)currhist[count];
        }
...
void DisplayGheAlgorithm(DISPLAY_PC_XPST_CONTEXT *pDpstContext)
{
    ...
    // Calculate Histogram change percentage. This is used to determine cutoff freq speed for phase in smoothening.
    pDpstContext->Algorithm.XpstAlgorithmDynamicData.HistogramChangePercentage = GetRelativeFrameBrightnessChange(pDpstContext);
    memcpy(pDpstContext->Algorithm.XpstAlgorithmDynamicData.PrevHistogram,
             pDpstContext->Algorithm.XpstAlgorithmDynamicData.Histogram, sizeof(pDpstContext->Algorithm.XpstAlgorithmDynamicData.Histogram));

    memset(&CurrSignature, 0, sizeof(XPST_GAMMA_SIG));
